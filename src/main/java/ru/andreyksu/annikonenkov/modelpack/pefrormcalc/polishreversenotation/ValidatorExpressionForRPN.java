...
public class ValidatorExpressionForRPN {
    ...
    private static final Logger log =
            LogManager.getLogger(ValidatorExpressionForRPN.class);

    public boolean validate(String str) {
        char[] chars = str.toCharArray();
        long time = System.nanoTime();

        if (validateOnPairBraces(chars) && isOrderedBraces(chars)
                && isContainedEmptyBraces(chars)
                && isContainedPairOperators(chars)
                && isPairedOperatorAndClosedBrace(chars)
                && isPairedOperatorAndOpenBrace(chars)
                && isOperatorBeginOrEnd(str)) {
     ...
     * 
     * @param chars - входной массив симолов
