...
public class PanelClock extends Panel {
	...
	/**
	 * Отрисовка часов; насечек и стрелок. Отрисовка происходит каждую секунду.
	 * Отрисовка вызывается из класса CalcController.
	 */
	public void paint(Graphics gr) {
		...
		if (init) {
			...
			xyHour = getXY(R2 - 43, hour, 12, dx, dy, -Math.PI / 2 + getOffsetForHours());
			...
			strr = String.format("%1$02d : %2$02d : %3$02d", hour, minut, seconds);
	...
	/**
	 * Метод нужен для смещения часовой стрелки в между часом на три положения.
	 * 
	 * @return
	 */
	...
	/**
	 * Метод позволяет получить координаты для отрисовки насечек и стрелок.
	 * 
	 * @param R
	 * @param k
	 * @param d
	...
	private int[] getXY(int R, int k, int d, int x, int y, double rad) {
		double tmp_param = (double) (k * 2 * Math.PI) / d;
		int x1 = (int) (R * (Math.cos(tmp_param + rad))) + x;
		int y1 = (int) (R * (Math.sin(tmp_param + rad))) + y;
		int[] xyCoord = { x1, y1 };
		return xyCoord;
	}
